...
const handleFiles = () => {
  const parser = [...fileInput.files].map((file) => {
  ...
  fechamentoPromise.then((dfCompletos) => {
    ...
    for (const [index, f] of dfCompletos.entries()) {
      for (const [index2, g] of dfCompletos.entries()) {
        if (index !== index2) {
          //filtra para que uma tabela não seja comparada consigo mesma
...
function criaTabela(tabela1, tabela2, totalmerged) {
  const dfmerged = tabela1
    .filter((item1) => {
      const item2 = tabela2.find(
        (item2) => item1["Produto"] === item2["Produto"],
      );
